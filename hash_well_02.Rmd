...
#assay <- "RNA"
metric1 <- "entropy"
metric2 <- "localStructure"
#group <- "orig.id"
#dim_red <- "INTEGRATED.RPCA"
dim_red_pre <- "PCA"
...
# Check if k is larger than the sets to be evaluated before locStructure
# Size of the batches must be higher than k
while (k > min(table(object@meta.data[[group]]))) {
  trimgroup <- names(table(object@meta.data[[group]]))[which(table(object@meta.data[[group]]) ==        
                                                               min(table(object@meta.data[[group]])))]
  Idents(object) <- group
  object <- subset(object, idents = setdiff(levels(Idents(object)), trimgroup))
...
HW_beffect2 <- evalIntegration(metrics = metric2, sce = scO, 
                              group = group, 
                              dim_red = dim_red_pre, 
                              dim_combined = dim_red,
                              n_dim = 50,n_combined = 50, k = k)
...
out_dataframe2 <- data.frame(LocStr <- HW_beffect2@colData@listData$overlap, 
                            sourceset <- rep(paste(assay, group, dim_red, metric2, sep = "_"), 
                                             length(HW_beffect2@colData@listData$overlap)))
colnames(out_dataframe2) <- c(metric2, "sourceset")

outlist <- list()
outlist[['entropy']] <- out_dataframe1
outlist[['LocStr']] <- out_dataframe2
...
print("sample done")
all_comp_out[['hash']] <- comp_eval(Obj_RNA, Obj_SCT, "HTO_maxID") #Hashtag
print("hashtag done")
all_comp_out[['hashwell']] <- comp_eval(Obj_RNA, Obj_SCT, "orig.id") #hashwell
print("pooled hash done")
